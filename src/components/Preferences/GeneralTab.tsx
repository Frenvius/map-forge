...
interface GeneralTabProps {
  ...
  data: { dir: string; onPick: () => void; onReset: () => void };
  onResetLayout: () => void;
...
const GeneralTab = ({ config, data, onResetLayout, onChange }: GeneralTabProps) => {
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex min-w-0 flex-col">
          ...
          <span className="truncate text-[10px] text-muted-foreground">{data.dir || 'Bundled (next to the app)'}</span>
        </div>
        <div className="flex shrink-0 gap-2">
          {data.dir && (
            <Button size="sm" variant="ghost" onClick={data.onReset}>
              Use default
            </Button>
          )}
          <Button size="sm" variant="outline" onClick={data.onPick}>
            Browse...
          </Button>
        </div>
      </div>
